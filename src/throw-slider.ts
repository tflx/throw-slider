...
export class ThrowSlider extends LitElement {
    ...
    @property({ type: Boolean }) center = false // active element is the one in the center of the container rather than th left edge
    ...
    connectedCallback() {
        ...
        defer(() => {
            const options: ThrowSliderOptions = {
                ...
                snap: !this.noSnap,
                paddingRight: getComputedStyle(this).getPropertyValue('--throw-slider-slide-gap'),
    ...
    private async init(options: ThrowSliderOptions) {
        ...
        this.loop = this.createLoop(items, {
            onChange: (element:HTMLElement, index:number) => { // when the active element changes, this function gets called.
                ...
                this.dispatchEvent(new CustomEvent('onChange', { detail: { currentItem:activeElement, currentIndex:index } }))
            },
            ...options
        });
    }
    ...
    private hasImages():boolean {
        return this.carouselItems.some((item:HTMLElement) => item.querySelector('img'))
    }

    private isAutoWidth():boolean {
        return window.getComputedStyle(this).getPropertyValue("--throw-slider-slide-width") === 'auto'
    }

    private async imagesLoaded():Promise<unknown> {
        const images:Array<HTMLImageElement> = this.carouselItems.map((item:HTMLElement) => item.querySelector('img') as HTMLImageElement)
        return await Promise.all(images.map((image:HTMLImageElement) => {
    ...
    private createLoop(items: Array<HTMLElement>, config: ThrowSliderOptions):gsap.core.Timeline {
        ...
        snap = config.snap === false ? (v:any) => v : gsap.utils.snap(Number(config.snap) || 1), // some browsers shift by a pixel to accommodate flex layouts, so for example if width is 20% the first element's width might be 242px, and the next 243px, alternating back and forth. So we snap to 5 percentage points to make things look more natural
        timeOffset = 0,
        ...
        tl.next = (vars: gsap.TweenVars | undefined) => toIndex(tl.current() + 1, vars);
        tl.previous = (vars: gsap.TweenVars | undefined) => toIndex(tl.current() - 1, vars);
        tl.times = times;
        ...
        if (config.draggable && typeof (Draggable) === "function") {
            ...
            let wrap = gsap.utils.wrap(0, 1),
                ratio: number, startProgress: number, draggable: globalThis.Draggable, lastSnap: number, initChangeX: number,
    ...
    public toggleAutoplay(force?: boolean) {
        if (force === undefined) !this.playing ? this.playTimeline() : this.pauseTimeline()        
    ...
    public next(options:ThrowSliderControlOptions) {
        this.loop.next(Object.assign({}, {duration: 0.4, ease: "power1.inOut"}, options));
    }
    ...
    public previous(options:ThrowSliderControlOptions) {
        this.loop.previous(Object.assign({}, {duration: 0.4, ease: "power1.inOut"}, options));
    }
    ...
    public goTo(index: number, options:ThrowSliderControlOptions) {
        this.loop.toIndex(index, Object.assign({}, {duration: 0.4, ease: "power1.inOut"}, options));
    }
